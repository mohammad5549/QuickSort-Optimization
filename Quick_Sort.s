...
.LFB2276:
    	.cfi_startproc
    	endbr64
    	movq    %rdi, %rax        # Load first pointer into %rax
    	movl    (%rax), %ecx      # Load value at first pointer into %ecx
    	movq    %rsi, %rdx        # Load second pointer into %rdx
    	movl    (%rdx), %ebx      # Load value at second pointer into %ebx
    	movl    %ebx, (%rax)      # Store second value into first pointer
    	movl    %ecx, (%rdx)      # Store first value into second pointer
    	ret                       # Return
    	.cfi_endproc
